...
export const fetchOrderHistories = async (
  query: OrderHistoriesControllerFindAllV1Data
) => {
...
export const useOrderHistories = (
  ...
  const { data, error, isLoading, mutate } = useSWR("orderHistories", () =>
    fetchOrderHistories(query)
  );
  return {
    data,
    error,
...
export const createOrderHistory = async (
  ...
  onSuccess: (res: OrderHistory) => void,
  onError: () => void
) => {
  try {
